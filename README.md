...
On each run (`/api/storefronts`, every 6 hours by default in `vercel.json`), each tracked seller's current ASIN catalog is fetched from Keepa's Seller API (no live-refresh `update` param is used, so this reads whatever Keepa already has rather than forcing a fresh collection — keeps the token cost small and predictable, roughly 1 token per tracked seller per run) and diffed against what was seen last run (`storefront:seen:{sellerId}` in Redis). The very first run for a newly added seller only baselines their existing catalog; it does not alert on all of it as "new."
