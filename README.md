...
A second, independent feature: watch specific Amazon seller storefronts and get a Discord alert the moment they list a new product, with an automatic attempt to find a cheaper Walmart source for it.

`AMAZON_TRACKED_SELLERS` is a comma-separated list of Amazon seller IDs, each optionally labeled: `A2L77EE7U53NWQ:Amazon Warehouse,A1B2C3D4E5F6G7`. A seller ID is found in the `seller=` parameter of that seller's Amazon storefront URL. `STOREFRONT_DISCORD_WEBHOOK_URL` is where alerts go — this is separate from student webhooks, since storefront tracking isn't tied to the student roster.

On each run (`/api/storefronts`, once daily by default in `vercel.json`, offset from the main sourcing cron — Vercel's Hobby plan only allows daily cron schedules), each tracked seller's current ASIN catalog is fetched from Keepa's Seller API (no live-refresh `update` param is used, so this reads whatever Keepa already has rather than forcing a fresh collection — keeps the token cost small and predictable, roughly 1 token per tracked seller per run) and diffed against what was seen last run (`storefront:seen:{sellerId}` in Redis). The very first run for a newly added seller only baselines their existing catalog; it does not alert on all of it as "new."

For each genuinely new ASIN (capped at `STOREFRONT_NEW_LISTINGS_PER_RUN_LIMIT`, default 20 per run), the product is fetched from Keepa and Walmart is searched by title for a plausible source. Only the top few search results get the expensive per-item detail-page re-check (search cards don't carry a UPC, only detail pages do), and a match only counts as a "qualified" deal card if it clears the exact same bar a normal qualified deal does: UPC match, identity/variant/quantity compatibility, the ROI/profit/sales-velocity thresholds, and Gemini's exact-match verification. Every new listing gets a Discord alert regardless; only ones with a verified, qualifying Walmart source get the full deal-card fields — everything else is a plain "new listing, no sourcing match found" notice.
