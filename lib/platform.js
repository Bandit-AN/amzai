...
const apparelTerms = /\b(?:t-?shirt|undershirt|shirt|pants|dress|shoe|sneaker|underwear|boxer briefs?|bra|jacket|swimsuit|swimwear|bathing suit|clothing|apparel)\b/i;
...
export async function confirmWalmartAvailability(deal) {
  ...
  return products.some((product) => String(product.itemId) === String(deal.itemId));
}
...
export async function analyzeCandidate(candidate) {
  ...
  // Apparel clearance URLs frequently resolve to a different size/style than
  // the advertised tile, and neither marketplace feed exposes enough variant
  // state to prove the exact purchasable SKU. Hold these back until we have a
  // variant identifier rather than presenting a potentially wrong lead.
  if (apparelTerms.test(candidate.title)) return { deal: null, rejection: 'unverified_apparel_variant' };
