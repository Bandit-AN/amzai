...
export function walmartUrlsForDailyRun(now = Date.now()) {
  const sources = walmartSourceUrls();
  const broadSources = sources.filter((value) => new URL(value).pathname !== '/search');
  const searchSources = sources.filter((value) => new URL(value).pathname === '/search');
  const day = Math.floor(Number(now) / 86400000);
  const maximumDailyDepth = Math.max(1, Math.min(6, config.walmartMaxPage));
  const pageDepth = ((day % maximumDailyDepth) + maximumDailyDepth) % maximumDailyDepth + 1;
  const budget = Math.max(1, config.walmartPagesPerRun);
  const selected = broadSources.slice(0, Math.min(4, budget)).map((value) => {
    const url = new URL(value);
    if (pageDepth > 1) url.searchParams.set('page', String(pageDepth));
    return url.href;
  });
  const searchBudget = Math.max(0, budget - selected.length);
  for (let offset = 0; offset < searchBudget && searchSources.length > 0; offset += 1) {
    selected.push(searchSources[((day * Math.max(1, searchBudget)) + offset) % searchSources.length]);
  }
  return selected;
}
...
export async function analyzeCandidate(candidate) {
  ...
  const products = await findKeepaProductsByCode(candidate.upc);
