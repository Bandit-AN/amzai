...
export async function getRunSummary(runId) {
  ...
  const funnel = {
    ...
    exactAmazonMatchFound: Number(funnelState[2] || 0),
    economicsPassed: Number(funnelState[3] || 0),
    stockConfirmed: Number(funnelState[4] || 0),
    automaticallyDelivered: Number(funnelState[5] || 0),
    manualReview: Number(funnelState[6] || 0),
  };
  const outcome = !finalized && !cancelled
    ? 'processing'
    : (deals.length > 0
      ? 'profitable_products_found'
      : (funnel.upcConfirmed === 0 ? 'identity_not_established' : 'no_profitable_products'));
  ...
  return {
    ...
    status: cancelled
      ? 'cancelled'
      : (finalized ? (meta.auditMode ? 'awaiting_audit' : 'finalized') : 'analyzing'),
...
function normalizeWalmartProduct(candidate) {
  ...
  const availability = first(candidate, [
    ...
    'product.availability', 'item.availabilityStatus', 'offers.availability',
  ]);
...
export function walmartSourceUrls() {
  ...
  const curated = walmartBase ? [
    ...
    ...[
      ...
      'clearance office school supplies',
      'clearance health beauty',
      'clearance kitchen dining',
      'clearance automotive',
      'clearance patio garden',
      'clearance seasonal',
    ].map((query) => {
...
export function automaticEligibilityReason(candidate) {
  ...
  if (variationHeavyTerms.test(String(candidate.title || '')) && !candidate.variantId) {
...
async function findKeepaProducts(searchTerms) {
  ...
  for (const term of terms) {
    ...
    try {
      ...
      const wrapped = new Error(`Keepa search ${error.response?.status || 'request'}: ${detail}`);
  ...
  try {
    ...
    const wrapped = new Error(`Keepa product ${error.response?.status || 'request'}: ${message}`);
    wrapped.response = error.response;
    wrapped.code = error.code;
    throw wrapped;
  }
...
function evaluateProduct(candidate, identification, product) {
  const canonicalWalmartTitle = identification.canonicalTitle || candidate.searchTitle || candidate.title;
...
export async function analyzeCandidate(candidate) {
  ...
  const products = await findKeepaProducts([candidate.upc]);
  ...
  return qualified.length
    ? {
      deal: { ...qualified[0], matchMethod: 'UPC' },
      rejection: null,
      diagnostics,
      funnel: { exactAmazonMatchFound: true, economicsPassed },
    }
    : {
      deal: null,
