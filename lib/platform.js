...
const manualReviewReasonLabel = (reason) => ({
  missing_sales_velocity: 'A real, identity-verified Amazon match was found, but Keepa has no sales-velocity data for this ASIN — confirm demand yourself before buying.',
  missing_upc: 'No UPC was found on the Walmart listing, so an exact Amazon match could not be verified automatically.',
  walmart_detail_lookup_error: 'The Walmart listing could not be re-verified (site or scraper error) — confirm it is still in stock at this price before buying.',
  walmart_detail_unverified: 'The Walmart detail page could not be confirmed.',
  walmart_unavailable: 'This item appears to be out of stock online.',
  unverified_variant: 'The exact size, color, or style could not be confirmed — verify the variant before buying.',
}[reason] || 'This candidate did not pass one of the automated verification steps.');

// Manual-review candidates have not cleared the identity/economics checks a
// qualified deal has — they are surfaced, not screened. The message and
// color are deliberately distinct from discordPayload's qualified-deal
// format so they can never be mistaken for a vetted recommendation.
export function manualReviewDiscordPayload(student, entries) {
  const embeds = entries.map((entry, index) => ({
    title: `${index + 1}. ${entry.title}`.slice(0, 256),
    url: entry.walmartUrl,
    color: 0x95a5a6,
    thumbnail: entry.imageUrl ? { url: entry.imageUrl } : undefined,
    fields: [
      { name: 'Walmart price', value: dollars(entry.currentPrice), inline: true },
      ...(entry.asin ? [
        { name: 'Amazon price', value: dollars(entry.amazonPrice), inline: true },
        { name: 'Gross price spread', value: `${Number(entry.roi).toFixed(1)}%`, inline: true },
        { name: 'Est. net after fees', value: dollars(entry.estimatedProfit), inline: true },
        { name: 'ASIN', value: entry.asin, inline: true },
      ] : []),
      { name: 'Why it needs review', value: manualReviewReasonLabel(entry.reason), inline: false },
      {
        name: 'Source',
        value: entry.asin ? `[Walmart](${entry.walmartUrl}) · [Amazon](${entry.amazonUrl})` : `[Walmart](${entry.walmartUrl})`,
        inline: false,
      },
    ],
  }));
  return {
    username: 'Syndicate AI Sourcing',
    content: `🔍 **MANUAL REVIEW** for ${student.name} — ${entries.length} candidate${entries.length === 1 ? '' : 's'} the automated screen could not fully verify today. `
      + 'These have **not** passed the profitability or identity checks a qualified deal has — review each one yourself before buying.',
    embeds,
  };
}

export function manualReviewDiscordPayloads(student, entries, entriesPerMessage = 4) {
  const chunks = [];
  for (let index = 0; index < entries.length; index += entriesPerMessage) {
    chunks.push(entries.slice(index, index + entriesPerMessage));
  }
  return chunks.map((chunk, index) => {
    const payload = manualReviewDiscordPayload(student, chunk);
