...
export async function fetchWalmartCatalog(
  ...
  // Fetched in parallel — sequentially, a source count anywhere near the
  // per-page worst-case (ScrapingAnt's browser-mode escalation can take up
  // to 60s) risks exceeding the calling function's own timeout budget.
  const candidateGroups = await Promise.all(
    targetUrls.map((url) => fetchOneWalmartSource(url, includeUnavailable)),
  );
