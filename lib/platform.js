...
async function fetchOneWalmartSource(url, includeUnavailable) {
  ...
  return normalizeWalmartPayload(response.data)
...
async function fetchWalmartSourcesWithConcurrencyLimit(targetUrls, includeUnavailable) {
  ...
  async function worker() {
    while (cursor < targetUrls.length) {
      ...
      cursor += 1;
      try {
        results[index] = await fetchOneWalmartSource(targetUrls[index], includeUnavailable);
      } catch (error) {
        results[index] = [];
        failures.push({
  ...
  if (failures.length === targetUrls.length) {
    const error = new Error(`All ${targetUrls.length} Walmart source requests failed`);
...
export async function fetchWalmartCatalog(
  ...
  const candidateGroups = await fetchWalmartSourcesWithConcurrencyLimit(targetUrls, includeUnavailable);
