...
export function walmartSourceUrls() {
  ...
  for (const source of [...configured, ...curated]) {
    ...
    unique.set(`${source.origin}${source.pathname}`, source.href);
  }
...
export async function analyzeCandidate(candidate) {
  ...
  const rejectionPriority = [
    'identity_mismatch', 'product_code_mismatch', 'variant_mismatch', 'quantity_mismatch',
    'blocked_brand', 'missing_amazon_price', 'price_spread', 'missing_sales_velocity', 'sales_velocity',
  ];
  ...
  qualified.sort((left, right) => (right.roi + Math.log10(right.estimatedMonthlySales) * 10)
