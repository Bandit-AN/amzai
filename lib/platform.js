...
const mandatoryBlockedBrands = ['LEGO', 'Barbie', 'Monster High', 'Apple'];
...
async function findKeepaProducts(searchTerm) {
  let search;
  try {
  ...
  const asins = [...new Set(rawResults.map((result) =>
...
export function listingVariantsCompatible(walmartTitle, amazonTitle) {
  ...
  if ((walmartVersions.length || amazonVersions.length)
    && setsConflict(walmartVersions.length ? walmartVersions : ['base'], amazonVersions.length ? amazonVersions : ['base'])) {
    return false;
  }
  ...
  if ((walmartConditions.length || amazonConditions.length)
    && setsConflict(walmartConditions.length ? walmartConditions : ['new'], amazonConditions.length ? amazonConditions : ['new'])) {
    return false;
  }
  ...
  if ((walmartModels.length > 0) !== (amazonModels.length > 0)) return false;
...
export async function analyzeCandidate(candidate) {
  ...
  const products = await findKeepaProducts(candidate.upc || identification.cleanSearchTerm);
  ...
  for (const product of products) {
    ...
    if (result.deal.estimatedProfit <= 0) {
  ...
  return qualified.length
    ? { deal: qualified[0], rejection: null }
    : { deal: null, rejection: furthestRejection };
}
...
function eligibleForStudent(deal, student) {
  return deal.roi >= Math.max(config.minimumRoi, student.minRoi)
    && (deal.estimatedProfit === undefined || Number(deal.estimatedProfit) > 0)
