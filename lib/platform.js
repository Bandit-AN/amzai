...
export async function getRunSummary(runId) {
  ...
  const funnel = {
    ...
    amazonUpcMatchFound: Number(funnelState[2] || 0),
    exactAmazonMatchFound: Number(funnelState[3] || 0),
    economicsPassed: Number(funnelState[4] || 0),
    stockConfirmed: Number(funnelState[5] || 0),
    automaticallyQualified: Number(funnelState[6] || deals.length || 0),
  ...
  const outcome = !finalized && !cancelled
    ...
    : (deals.length > 0
      ...
      : (funnel.upcConfirmed === 0
        ...
        : (funnel.exactAmazonMatchFound === 0
