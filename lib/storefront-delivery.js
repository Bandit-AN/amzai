...
export async function processStorefront({ store, redis, fetchSeller, hydrate, blocked,
  ...
  try {
    ...
    if (!state.pending.length && !state.outbox) return await finish({ status: state.status, pending: 0 });
    if (!state.outbox) {
    ...
    state.error = null;
    state.status = state.pending.length ? 'pending' : 'up_to_date';
    return await finish({ status: state.status, delivered: outbox.count, pending: state.pending.length });
  } catch (error) {
