...
export async function processStorefront({ store, redis, fetchSeller, hydrate, blocked,
  payloads, send, ttl, now = () => new Date().toISOString() }) {
  ...
  try {
    ...
    if (!state.pending.length && !state.outbox) return { status: state.status, pending: 0 };
    if (!state.outbox) {
    ...
    state.lastDelivery = { at: now(), messageId: outbox.messageId, asins: outbox.asins };
    state.outbox = null;
    state.error = null;
    state.status = state.pending.length ? 'pending' : 'up_to_date';
    await save(state);
    return { status: state.status, delivered: outbox.count, pending: state.pending.length };
  } catch (error) {
