...
import { refreshOrdersFromGmail } from './email-tracking.js';
...
export async function handleOrderTrackingCron(request, response) {
  ...
  try {
    const gmail = await refreshOrdersFromGmail();
    if (!config.easyPostApiKey) {
      return jsonResponse(response, 200, {
        ok: true,
        configured: gmail.configured,
        provider: 'gmail',
        checked: gmail.checkedConnections,
        updated: gmail.updated,
        gmail,
        message: gmail.configured
          ? 'Gmail shipment messages checked; carrier API tracking is optional'
          : 'Connect Gmail in the Order tracking tab to enable daily email updates',
      });
    }
